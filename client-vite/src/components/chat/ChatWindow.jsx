    ...
    <div
      className={
        embedded
          ...
          : `fixed bottom-40 right-20 z-50 bg-white shadow-2xl rounded-xl border border-slate-200
             transition-all duration-200 ${minimized ? 'h-12 w-80' : 'w-[92vw] max-w-[360px] md:max-w-[420px] h-[60vh]'}`
      }
      role="dialog"
      aria-label="Cửa sổ chat hỗ trợ"
    >
      {/* Header */}
      <div className={`flex items-center justify-between px-3 py-2 ${embedded ? 'border-b' : 'border-b'}`}>
        ...
        {!embedded && (
          <div className="flex items-center gap-2">
            ...
            >
              {minimized ? (
                <svg className="h-5 w-5" viewBox="0 0 24 24" fill="none"><path d="M6 18L18 6" stroke="currentColor" strokeWidth="2"/></svg>
              ) : (
                <svg className="h-5 w-5" viewBox="0 0 24 24" fill="none"><path d="M6 6l12 12" stroke="currentColor" strokeWidth="2"/></svg>
              )}
            </button>
            <button
              className="p-2 rounded hover:bg-slate-100 active:scale-95 transition"
              ...
              title="Đóng"
            >
              <svg className="h-5 w-5" viewBox="0 0 24 24" fill="none"><path d="M6 6l12 12M6 18L18 6" stroke="currentColor" strokeWidth="2"/></svg>
            </button>
          </div>
        )}
      </div>
      ...
      {/* Body */}
      {!minimized && (
        <div className={embedded ? "flex flex-col flex-1 min-h-0" : "flex flex-col h-[calc(60vh-48px)]"}>
          {/* Danh sách tin nhắn */}
          <div ref={listRef} className="flex-1 min-h-0 overflow-y-auto p-3 space-y-2 bg-slate-50">
            {loading ? (
              ...
              messages.map((m) => {
                const isMe = userId && (m.sender === userId || m.sender?._id === userId || m.sender?.toString?.() === userId);
                return (
                    ...
                    >
                      {m.type === 'image' ? (
                        <div className="space-y-1">
                          ...
                          <img src={m.imageUrl} alt="image" className="rounded-md max-h-56 object-cover" loading="lazy" />
                        </div>
                      ) : (
